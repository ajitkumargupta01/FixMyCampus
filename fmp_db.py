...
# #------------------------------------------------
# Host: sql12.freesqldatabase.com
# Database name: sql12774989
# Database user: sql12774989
# Database password: acxEkHFzcu
# Port number: 3306
...
def fetch_user(roll_no):
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT first_name, last_name, email, mob_num, gender, roll_no FROM users WHERE roll_no = %s", (roll_no,))
        user = cur.fetchone()
        return user
...
def check_user(roll_no, password):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM users WHERE roll_no=%s AND password=%s AND is_banned=FALSE", (roll_no, password))
...
def all_user():
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT first_name, last_name, email, password, mob_num, gender, roll_no, is_banned FROM users")
    users = cur.fetchall()
    conn.close()
    return users
...
def ban_user(roll_no):
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("UPDATE users SET is_banned = TRUE WHERE roll_no = %s", (roll_no,))
        conn.commit()
        conn.close()
...
def unban_user(roll_no):
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("UPDATE users SET is_banned = FALSE WHERE roll_no = %s", (roll_no,))
        conn.commit()
        conn.close()
...
def get_banned_users():
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("SELECT roll_no, first_name, last_name FROM users WHERE is_banned = TRUE")
        banned_users = cur.fetchall()
        conn.close()
        return banned_users
    except Exception as e:
        print(f"[Get Banned Users] Error: {e}")
        return []
...
def get_audit_logs(limit=50):
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("SELECT action, details, timestamp FROM audit_logs ORDER BY timestamp DESC LIMIT %s", (limit,))
        logs = cur.fetchall()
        conn.close()
        return logs
    except Exception as e:
...
def create_audit_logs_table():
    try:
        ...
        print(f"[Create Audit Logs] Error: {e}")
    finally:
